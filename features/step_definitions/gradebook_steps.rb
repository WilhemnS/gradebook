...
Given("I am non-signed user") do
    @user = create(:user)
    #pending # Write code here that turns the phrase above into concrete actions
end

Then("I should not be able to see View Gradebook") do
    expect(page).not_to have_content("View Gradebook")
    #pending # Write code here that turns the phrase above into concrete actions
end

Then("I should see Student ID") do
    expect(page).to have_content("Student ID")
end

Given("I sign in as a teacher") do
    @user = User.create(email: "teacher@gmail.com",password: "default123",account_id:1)
    visit new_user_session_path
  ...
  Given("I sign in as a TA") do
    @user = User.create(email: "TA@gmail.com",password: "default123",account_id:0)
    visit new_user_session_path
    fill_in "user_email", with: @user.email
    fill_in "user_password", with: @user.password
    click_on "Log in"
  ...
  Then("I should see the smallest grade first") do
    # Write code here that turns the phrase above into concrete actions
    @grades = Grade.order(student_grade: :ASC)
    
    within all('table tr')[1] do
        expect(page).to have_content(@grades.first.student_grade)
    end
  end

  Then("I should see the largest grade first") do
    # Write code here that turns the phrase above into concrete actions
